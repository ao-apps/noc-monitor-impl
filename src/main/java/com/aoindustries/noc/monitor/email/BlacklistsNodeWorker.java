...
class BlacklistsNodeWorker extends TableResultNodeWorker<List<BlacklistsNodeWorker.BlacklistQueryResult>, Object> {
  ...
  class SmtpBlacklist extends BlacklistLookup {
    ...
    public BlacklistQueryResult call() throws Exception {
      ...
      if (mxLookup.getResult() != Lookup.SUCCESSFUL) {
        throw new IOException(domain + ": MX lookup failed: " + mxLookup.getErrorString());
      }
      Record[] mxRecords = mxLookup.getAnswers();
      // Pick a random MX
      if (mxRecords.length == 0) {
        throw new IOException(domain + ": No MX records found");
      }
      MXRecord mx;
      if (mxRecords.length == 1) {
        mx = (MXRecord) mxRecords[0];
      } else {
        mx = (MXRecord) mxRecords[RootNodeImpl.fastRandom.nextInt(mxRecords.length)];
      }
      // Lookup the IP addresses
      ...
      if (device == null) {
        throw new SQLException(ipAddress + ": Device not found");
      }
      Server linuxServer = device.getHost().getLinuxServer();
      if (linuxServer == null) {
        throw new SQLException(ipAddress + ": Server not found");
      }
  ...
  protected List<BlacklistQueryResult> getQueryResult() throws InterruptedException, Exception {
    ...
    for (final BlacklistLookup lookup : lookups) {
      ...
      if (oldResult == null) {
        ...
        if (timeSince < 0) {
          timeSince = -timeSince; // Handle system time reset
        }
  ...
  public AlertLevelAndMessage getAlertLevelAndMessage(AlertLevel curAlertLevel, TableResult result) {
    ...
    // if (highestAlertLevel.compareTo(AlertLevel.MEDIUM)>0) {
